...
def init_auth(file):
	(CONSUMER_KEY, CONSUMER_SERET, OAUTH_TOKEN, OAUTH_TOKEN_SECRET) = open(file, 'r').read().splitlines()
	auth_obj = requests_oauthlib.OAuth1(CONSUMER_KEY, CONSUMER_SERET, OAUTH_TOKEN, OAUTH_TOKEN_SECRET)
	if verify_credentials(auth_obj):
		print('Validated credentials OK')
		return auth_obj
	else:
		print('Credentials validation ailed')
		sys.exit(1)
